...
console.log("================================================================");
console.log("[INFO] TELEGRAM CHANNEL ID LISTENER STARTED");
console.log("================================================================");
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (!discoveredChannels.has(chatId)) {
          ...
          console.log(`[INFO] [Update from "${title}" (${chatId})]: ${text}`);
        }
      }
    } catch (err) {
      if (err.code !== "ECONNABORTED" && err.response?.status !== 408) {
        console.error("[WARN] Polling error:", err.response?.data || err.message);
